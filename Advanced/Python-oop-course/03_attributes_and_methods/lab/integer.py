class Integer:
    def __init__(self, value):
        self.value = value

    @classmethod
    def from_float(cls, value):
        if not type(value) == float:
            return "value is not a float"
        return cls(int(value))

    @classmethod
    def from_string(cls, value):
        if not type(value) == str:
            return "wrong type"
        return cls(int(value))

    @classmethod
    def from_roman(cls, value):
        roman_vals = {'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000}
        new_value = 0
        for i in range(len(value)):
            if i > 0 and roman_vals[value[i]] > roman_vals[value[i - 1]]:
                new_value += roman_vals[value[i]] - 2 * roman_vals[value[i - 1]]
            else:
                new_value += roman_vals[value[i]]
        return cls(new_value)

    def add(self, integer):
        if not isinstance(integer, Integer):
            return "number should be an Integer instance"
        return self.value + integer.value


# Test code
first_num = Integer(10)
second_num = Integer.from_roman("IV")

print(Integer.from_float("2.6"))
print(Integer.from_string(2.6))
print(Integer.from_float(2.6))
print(Integer.from_string('5'))
print(first_num.add(second_num))
